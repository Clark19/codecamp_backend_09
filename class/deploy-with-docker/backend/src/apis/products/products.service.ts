...
export class ProductsService {
  ...
  async findAll() {
    ...
    // });
    const result = await this.productsRepository.find({
      relations: ['productSaleslocation', 'productCategory', 'productTags'], // 조회(fetch)시엔 등록한 테이블 조인해서 리턴하므로 create시와 다르게 카테고리 name 컬럼값들을 원하는대로 받을수있다.
    });
  ...
  async create({ createProductInput }) {
    ...
    // 2-2) 상품태그 등록
    // 프론트에서 #붙여서 보내준다고 가정: ex) ["#태그1", "#태그2"]
    const tags = [];
    // productTags.forEach(async (productTag) => {
    //   const tagname = productTag.replace('#', '');
    for (let i = 0; i < productTags.length; i++) {
      const tagname = productTags[i].replace('#', '');
      const prevTag = await this.productsTagsRepository.findOne({
        where: { name: tagname },
      });
      // console.log('00   tags: ', prevTag);

      // 이미 태그가 존재하는 경우
      if (prevTag) {
        tags.push(prevTag);
        console.log('00-1   prevTag: ', prevTag);
      } else {
        // 태그가 존재하지 않는 경우
        // !! for 문 안에서 await 사용하면 안됨. 안티패턴임.(나중에 Promise.all로 바꿀예정)
        const newTag = await this.productsTagsRepository.save({
          name: tagname,
        });
        tags.push(newTag);
        console.log('00-2   newTag: ', newTag);
      }
    }
    // });

    console.log('222222   tags: ', tags);
  ...
  async update({ productId, updateProductInput }) {
    ...
    // save 안에 id 있으면 update, 없으면 insert
